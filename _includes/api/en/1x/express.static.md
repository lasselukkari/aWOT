<h3 id='aWOT.static' class='h2'>aWOT.static(root, [options])</h3>

This is a built-in middleware function in aWOT.
...
See also the [example below](#example.of.aWOT.static).
...
| `etag`        | Enable or disable etag generation <br/><br/>NOTE: `aWOT.static` always sends weak ETags. | Boolean | `true` |
...
For more information, see [Serving static files in aWOT](/starter/static-files.html).
...
<h4 id='example.of.aWOT.static'>Example of aWOT.static</h4>

Here is an example of using the `aWOT.static` middleware function with an elaborate options object:
...
app.use(aWOT.static('public', options))
```
